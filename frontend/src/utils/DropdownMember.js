...
export default function DropdownMember(props) {
  ...
  async function changeAttendance(newValue, meeting) {
    const res = await fetch("https://${process.env.REACT_APP_HOSTNAME}/update_attendance", {
        method: "POST",
        headers: {
  ...
  async function changeAdmin(){
    await fetch(`https://${process.env.REACT_APP_HOSTNAME}/${props.member.admin?"revokeAdmin":"admin"}`, {
      method: "POST",
      headers: {
          "Content-Type": "application/json",
          authorization: "Bearer " + localStorage.getItem("@attendanceToken"),
